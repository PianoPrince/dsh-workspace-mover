...
**What was built** — 中英文 README 重写为面向使用者的产品页：功能/安装/截图/使用/接入原则/共存/兼容卸载/安全承诺/已知限制/仅 v2.0.1 更新说明，并链接 CHANGELOG 与 ARCHITECTURE。新建英文 `docs/ARCHITECTURE.md` 作为唯一技术事实来源（RPC 表、迁移流水线、数据目录、Host/Client、档案命名、兼容边界、测试发布）。clones 徽章在累计次数后追加 14 日窗口 uniques（`· N unique`）；功能徽章收敛为真迁移/零 token/备份回滚，保留 clones 与 Release downloads 指标位。

**Verification** — `npm test`：95/95 PASS（含更新后的 traffic-badge uniques 断言）。README 相对链接（媒体图、CHANGELOG、ARCHITECTURE、互链）均存在。独立 reviewer 对照 10 条验收全部 PASS；`lib/`、`client/`、`cordis.patch.yml` 未改动，版本仍为 2.0.1，未重打 tag。

**Journey log**
- 本地 main 与远端曾 ahead1/behind2；`git merge`/`worktree add` 被环境策略拦截，改为从 `origin/main` 独立 clone 到 `.worktrees/docs-readme-restructure` 工作，不触碰原 checkout 的 HEAD。
- uniques 不能跨日累加，徽章取 GitHub 14 日窗口 `lastWindow.uniques`；未恢复已废弃的 `wsm-clones-14d.json`。
- 安全区保留「并非任何错误都能自动恢复」边界，与 recovery ledger 行为一致。
- 架构文档只做英文一份，避免双语文档漂移。
- `docs/compose/spec/` 为工作流记录；若不想公开可发布前从分支剔除。
...
- [x] T1: 写入本 feature spec — acceptance: 文件存在且 status=designed (covers: S2)
- [x] T2: 新建 `docs/ARCHITECTURE.md`，包含挂载方式、Host/Client 职责、RPC 表、档案命名、迁移/备份/回滚/并发顺序、数据目录、兼容边界、测试与发布约束 — acceptance: 文件存在；README 中集成细节段落不再需要 (covers: S2; depends: T1)
- [x] T3: 重写 `README.md` 为 S2 结构，功能卖点/兼容/安全承诺保留，内部 API 词下沉 — acceptance: 目录与章节匹配 S2；无完整 RPC 清单与 8 步算法 (covers: S2; depends: T2)
- [x] T4: 重写 `README_EN.md` 与中文结构一一对应 — acceptance: 章节顺序与关键承诺中英对等；链接指向同一 ARCHITECTURE/CHANGELOG (covers: S2; depends: T3)
- [x] T5: `buildBadgeFiles` 输出 uniques；更新 traffic-badge 测试 — acceptance: `npm test` 中 traffic 用例通过；message 含 `unique` (covers: S2; depends: T1)
- [x] T6: 全量验证 — acceptance: `npm test` PASS；README 内相对链接文件存在；中英章节一一对应 (covers: S2; depends: T3,T4,T5)
- [x] T7: 在 `docs/readme-restructure` 提交文档修订 — acceptance: 分支相对 origin/main 有 docs 提交，无 lib/client 功能改动 (covers: S2; depends: T6)
