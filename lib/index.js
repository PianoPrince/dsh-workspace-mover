	...
	try {
		...
		try {
			...
			for (const e of allOwners) {
				try { if (!e.record.sessionIds.includes(sessionId)) await e.attachSession?.(sessionId); } catch { /* ignore */ }
			}
		} catch (rollbackErr) {
			recordRecovery(ctx, {
...
export function apply(ctx) {
	...
	const mount = (owner) => {
		...
		const dispose = scopedConnection.rpc.handle(CHANNEL, async (endpoint, payload = {}, signal) => {
