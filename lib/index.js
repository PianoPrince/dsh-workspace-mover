 ...
 * 目标目录的任何残骸（上次失败半成品、rename 遇 ENOTEMPTY/EEXIST）一律先清再搬，
 * 复制失败也清理目标——调用方还原源文件后磁盘零残留，重试幂等。
 * copyImpl 可注入（测试模拟复制中途失败）。
 */
function moveDir(srcDir, dstDir, copyImpl = cpSync) {
	...
	try {
		...
		try { rmSync(dstDir, { recursive: true, force: true }); } catch { /* 尽力清理 */ }
		throw err;
	}
