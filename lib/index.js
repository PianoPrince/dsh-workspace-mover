...
/**
 * 列出已归档会话：标题/cwd 取自档案头，归属取自原始记账（官方归档从不摘记账槽，
 * 所以归属工作区仍在）；cwd 匹配到别的工作区的行给出归位建议（与 scanSessions 同款判定）。
 */
async function listArchivedSessions(ctx) {
	...
	for (const sessionId of archivedIds) {
		...
		const suggest = matching && (!owner || matching.id !== owner.id) ? matching : null;
		items.push({
			sessionId,
			title: typeof header?.title === 'string' && header.title.trim() ? header.title.trim() : '未命名会话',
			cwd,
			ownerWorkspaceId: owner?.id ?? null,
			ownerTitle: owner?.title ?? null,
			suggestedWorkspaceId: suggest?.id ?? null,
			suggestedTitle: suggest?.title ?? null
		});
	}
