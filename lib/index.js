...
async function scanSessions(ctx) {
	...
	const selected = metas.slice(0, SCAN_MAX_ITEMS);
...
async function cleanupOldData(ctx, { days = 30, dryRun = false } = {}) {
	const keepDays = Math.min(3650, Math.max(1, Number(days) || 30)); // 输入钳制：1–3650 天
...
/** 非空字符串 id 校验（≤300 字符）。 */
function validId(value, name = 'id') {
...
async function deleteSessionToTrash(ctx, payload = {}, hooks = {}) {
	...
	try {
		return await deleteSessionToTrashLocked(ctx, sessionId, hooks);
	} finally {
		release();
	}
}
...
export { name, inject, CHANNEL, projectKey, encodeSegment, sessionDir, artifactPath, moveDir, stashBackup, verifyRelocatedArtifact, deleteSessionToTrash, moveSession, moveSessionEntry, validId, SCAN_MAX_ITEMS };
