 ...
 */
function verifyRelocatedArtifact(dstArtifact, sessionId, expectedCwd) {
	const header = readHeader(readFileSync(dstArtifact));
...
async function moveSession(ctx, { sessionId, targetWorkspaceId, sessionTitle, recordHistory = true }) {
	...
	try {
		...
		throw new Error(`target not writable: ${err?.message ?? err}`);
	}
	try {
		...
		if (typeof st.bfree === 'number' && st.bfree * Number(st.bsize || 4096) < needed) {
			throw new Error(`insufficient disk space at target (need ~${needed} bytes, free ~${st.bfree * Number(st.bsize || 4096)})`);
		}
	} catch (err) {
		if (/insufficient disk space/.test(String(err?.message ?? err))) throw err;
	...
	try {
		...
		for (const e of allOwners) { try { if (!e.record.sessionIds.includes(sessionId)) await e.attachSession?.(sessionId); } catch { /* ignore */ } }
	...
	try {
		...
		try {
			verifyRelocatedArtifact(dstArtifact, sessionId, targetPath);
		} catch (verifyErr) {
...
async function retryTask(ctx, { taskId } = {}) {
	...
	for (const item of retryable) {
		...
		try {
			const result = await moveSession(ctx, { sessionId: item.sessionId, targetWorkspaceId: task.targetWorkspaceId, sessionTitle: item.title ?? undefined, recordHistory: false });
			item.state = 'done';
			item.error = null;
			item.lastError = null;
 ...
 */
async function cleanupOldData(ctx, { days = 30, dryRun = false } = {}) {
	...
	for (const { sessionId, title } of affected) {
		...
		try {
			...
			try {
				...
				try {
					verifyRelocatedArtifact(dstArtifact, sessionId, targetCanon);
				} catch (verifyErr) {
 ...
 */
async function deleteSessionToTrash(ctx, { sessionId } = {}, hooks = {}) {
	const { moveImpl = moveDir, writeManifestImpl = (path, manifest) => writeFileSync(path, JSON.stringify(manifest, null, 2)) } = hooks;
	if (typeof sessionId !== 'string' || sessionId.length === 0) throw new Error('sessionId required');
	const registry = ctx.workspaceRegistry;
	const persistence = ctx.sessionPersistence;
	if (!registry || !persistence) throw new Error('workspace registry or session persistence service unavailable');
...
export { name, inject, CHANNEL, projectKey, encodeSegment, sessionDir, artifactPath, moveDir, stashBackup, verifyRelocatedArtifact, deleteSessionToTrash };
