...
export function openInFileManager(path, spawnFn = spawn) {
	if (process.platform === 'win32') {
		// 后台进程直接 spawn explorer.exe 时新窗口经常落在后台（无前台激活权）；
		// 走 Shell COM 的 ShellExecute 语义让资源管理器窗口正常置前。
		// PowerShell 单引号字符串仅需把内部单引号翻倍，对任意路径安全。
		const script = `(New-Object -ComObject Shell.Application).Open('${path.replace(/'/g, "''")}')`;
		const child = spawnFn('powershell.exe', ['-NoProfile', '-WindowStyle', 'Hidden', '-Command', script], { detached: true, stdio: 'ignore' });
		child?.unref?.();
