...
- **修复严重 bug**：多选拖拽可能移动未被选中的会话——官方隐藏的空白/归档会话在成员数组里占位却没有行，顺序对齐从缺口处整体错位。现在行→会话 id 改从行自身的 React props 读取（官方 dragstart 写入拖载数据的同一来源），对齐仅作兜底
- **修复置顶 bug 未生效**：v0.6.1/v0.6.2 的排序修复在真机上从未执行过（fiber 找不到目标 / 调用了客户端不存在的 API）。改为按排序账本自身成员 + 被移动会话直接算出正确顺序写入，已通过真实拖拽全链路验证
- 新增批量多选实拍截图

### v0.6.2 · 2026-08-28

- 「最近更新」重排改走官方槽位系统的 store 实例正式通道（v0.6.1 的 fiber 爬升实测未生效）：时间戳齐全时直接写入正确的最近更新顺序，否则清空账本交官方全量重排；flat 模式与手动排序绝不触碰
- 设置页撤回 / 补账 / 重新归组后主动重拉工作区基线——会话即时回到原分组，不再先落「未分组」等刷新
- Ctrl+点击开始多选时自动带上当前打开的会话：正开着 A、Ctrl+点 B，一步选中 {A, B}

### v0.6.1 · 2026-08-28

- 批量移动在「最近移动」聚合成一条记录（内部记下每个会话的来源分组），一键整批撤回；来源分组已删的会话保留在记录中可重试
- 普通点击会话行自动退出多选，不再残留"已选 X 个"徽章；Esc 在输入框聚焦时也能清空多选
- 修复「最近更新」排序下移动的会话被顶到最前：官方侧边栏把账本里没见过的会话当作"新活跃"置顶并记住；现在每次移动/撤回后自动经官方 store action 清空该分组的排序账本，触发与手动切换排序等价的全量按真实时间重排（仅限「最近更新」模式，手动排序的自定义顺序绝不动）
