...
- "Open folder" on Windows: spawning explorer.exe directly opened the window behind the foreground (background processes have no foreground-activation rights), while ShellExecute-based alternatives (PowerShell `Shell.Application.Open` / `Start-Process`) silently do nothing when spawned from the harness host. The window now opens via explorer.exe and, ~0.8s later, is brought to the front via `WScript.Shell.AppActivate` on the folder title (verified programmatically via `GetForegroundWindow`).
